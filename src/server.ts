...
+'</div><div style="margin-top:10px;font-size:13px;color:#2dd4bf;font-weight:700">→ '+(th?'หลาย agent รวมกัน เห็นสิ่งที่ตัวเดียวไม่เห็น · agent โกหกบังคับผลไม่ได้ · ตรวจซ้ำ '+(j.verified?'✓':'✕'):'the swarm sees what no single agent could · a liar cannot swing the verdict · verified '+(j.verified?'✓':'✕'))+'</div>';
document.getElementById('flabout').innerHTML=flabShell(col,(th?'ใบรับรองหลักฐานรวมหมู่ (Swarm)':'SWARM-EVIDENCE CERTIFICATE'),(th?'รวมหลักฐานหลาย agent อย่างเชื่อถือได้ ทนต่อ agent โกหก — สิ่งที่ LLM ทำไม่ได้':'pool many agents trustworthily, robust to a liar — an LLM cannot'),body,(th?'พิสูจน์แล้ว: รวมหมู่เจอ weak gain 61% (เดี่ยว 29%) · null ≤ α (0.1%) · agent โกหก 10⁶× ถูกคัดออก 100% บังคับผลผิดไม่ได้ · เซ็น Ed25519':'proven: pooling detects a weak gain 61% (single 29%) · null ≤ α (0.1%) · a liar inflating 10⁶× is excluded 100% and cannot force a false verdict · Ed25519-signed'));}

...
var binfo='<div id="braininfo" style="margin-top:9px;padding:10px 13px;background:#f5f3ff;border:1px solid #e7e0ff;border-radius:11px;font-size:13.5px;color:#33344e">⭐ '+(th?'จุดที่ดีที่สุด: ':'best so far: ')+'<b>'+brec+'</b> → '+(th?'คะแนน ':'score ')+(+N[t.root].value).toFixed(2)+'</div>';
el.style.display='block';
el.innerHTML='<div style="background:rgba(255,255,255,.82);backdrop-filter:blur(12px);border:1px solid #ece8fb;border-radius:20px;padding:20px 22px;box-shadow:0 26px 58px -34px rgba(70,55,160,.5)"><div style="display:flex;align-items:baseline;gap:9px;flex-wrap:wrap;margin-bottom:3px"><span style="font-size:13px;font-weight:800;letter-spacing:.4px;text-transform:uppercase;color:#6d28d9">🧠 '+(th?'สมองการค้นพบ':'Discovery brain')+'</span><span style="font-size:12.5px;color:#8890a8">'+(th?'แต่ละจุด = 1 การทดลอง · เส้นไหลขึ้นสู่ "สูตรที่ดีที่สุด" (จุดใหญ่เรืองแสง)':'each dot = one experiment · branches flow up to the best recipe (the big glowing dot)')+'</span></div><div style="font-size:12px;color:#6d28d9;font-weight:600;margin-bottom:4px">👆 '+(th?'แตะ/คลิกจุดไหนก็ได้ → สูตร+คะแนนของจุดนั้นจะขึ้นในกล่องด้านล่าง (มือถือก็แตะได้)':'Tap/click any dot → its recipe + score appears in the box below (works on mobile)')+'</div><svg viewBox="0 0 '+W+' '+H+'" style="width:100%;height:auto;display:block;cursor:pointer" onclick="gBrainClick(event)">'+edges+'<g id="brainpath"></g>'+pulse+nodes+labels+'</svg>'+binfo+'<div style="font-size:11.5px;color:#9aa0b8;margin-top:7px">'+(th?'จุดสว่าง/ใหญ่ = คะแนนสูง · 🟢 จุดเรืองแสง = สูตรที่ดีที่สุด · '+N.length+' การทดลอง':'brighter / bigger = higher score · 🟢 glowing dot = the best recipe · '+N.length+' experiments')+'</div></div>';}

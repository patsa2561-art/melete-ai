...
var panel=function(p){var c=COL[p.verdict]||'#94a3b8';var wit='';if(p.witness&&p.witness.length){var ws=p.witness.map(function(w){return (th?'ปุ่ม#':'knob #')+w.knob+': '+(th?'ขอ ':'asked ')+w.asked+' ('+(w.side==='above'?'>':'<')+' '+(th?'ค่าสูงสุด/ต่ำสุดที่วัด ':'sampled limit ')+w.sampledLimit+')';}).join(' · ');wit='<div style="font-size:11px;color:#fbbf24;margin-top:8px;font-family:ui-monospace,monospace;line-height:1.5">'+(th?'พยานแยกระนาบ: ':'separating witness: ')+ws+'</div>';}return '<div style="flex:1;min-width:200px;background:rgba(255,255,255,.04);border:1px solid '+c+'55;border-radius:14px;padding:14px 16px"><div style="font-size:11px;letter-spacing:.5px;text-transform:uppercase;color:#9fb0d0;margin-bottom:6px">'+p.label+'</div><div style="font-size:13px;font-family:ui-monospace,monospace;color:#cdd6ee">x* = ['+p.recommended.join(', ')+']</div><div style="font-size:16px;font-weight:800;color:'+c+';margin-top:9px">'+(LBL[p.verdict]||p.verdict)+'</div><div style="font-size:11px;color:#8a98b8;margin-top:4px">'+(th?'อัตราส่วนระยะใกล้สุด ':'support ratio ')+p.supportRatio+'</div>'+wit+'</div>';};
var body='<div style="font-size:13px;color:#cdd6ee;margin:2px 2px 12px">'+(th?('สุ่มวัด '+j.designSize+' จุด (สองกระจุก เว้นช่องตรงกลาง). ตรวจคำแนะนำ 3 แบบว่าอยู่ "ในหลักฐาน" หรือเป็นการเดานอกข้อมูล — ตัวที่อยู่นอกกล่องมี "พยานแยกระนาบ" พิสูจน์ว่าอยู่นอก convex hull แน่นอน'):('a design of '+j.designSize+' measured settings (two clusters, a gap between). Three recommendations are checked for whether they sit INSIDE the evidence — the out-of-box one ships a separating-axis WITNESS that exactly proves it is outside the convex hull'))+'</div><div style="display:flex;gap:12px;flex-wrap:wrap">'
+panel(j.insideCluster)+panel(j.interiorVoid)+panel(j.beyondBox)
+'</div><div style="margin-top:10px;font-size:13px;color:#2dd4bf;font-weight:700">→ '+(th?'อย่าเชื่อคำแนะนำที่อยู่นอกหลักฐาน · พยานตรวจซ้ำเองได้ ไม่ต้องส่งข้อมูลทั้งชุด':'never trust a recommendation outside the evidence · the witness re-checks on its own, without re-shipping the design')+'</div>';
document.getElementById('flabout').innerHTML=flabShell(col,(th?'ใบรับรองขอบเขตข้อมูล (Extrapolation-Guard)':'EXTRAPOLATION-GUARD CERTIFICATE'),(th?'คำแนะนำอยู่ในข้อมูลหรือเดาเอา — สิ่งที่ LLM ทำไม่ได้':'is the recommendation supported or a guess — an LLM cannot'),body,(th?'พิสูจน์แล้ว: นอกกล่อง→extrapolation 100% พร้อมพยานแยกระนาบ · ไม่เคย false-flag จุดที่วัดจริง · ช่องว่างในกล่องจับได้ · ปลอม SUPPORTED ถูกจับ · เซ็น Ed25519':'proven: out-of-box → EXTRAPOLATION 100% with a separating witness · never false-flags a sampled point · catches interior voids · a fake SUPPORTED is caught · Ed25519-signed'));}

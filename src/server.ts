...
var panel=function(title,p,note){var robust=(p.verdict==='ROBUST');var c=robust?'#34d399':'#fb7185';var m=p.breakdown+(p.atLeast?'+':'');return '<div style="flex:1;min-width:220px;background:rgba(255,255,255,.04);border:1px solid '+c+'55;border-radius:14px;padding:14px 16px"><div style="font-size:11px;letter-spacing:.5px;text-transform:uppercase;color:#9fb0d0;margin-bottom:6px">'+title+'</div><div style="font-size:34px;font-weight:800;color:'+c+';text-shadow:0 0 16px '+c+'55;line-height:1">'+m+'</div><div style="font-size:11px;color:#8a98b8;margin-top:2px">'+(th?('การวัดที่ถูกปลอม จาก '+p.ofMeasurements+' จุด เพื่อพลิกผล'):('corrupted of '+p.ofMeasurements+' measurements would flip it'))+'</div><div style="font-size:15px;font-weight:800;color:'+c+';margin-top:9px">'+(robust?(th?'🪨 ทนทาน (ROBUST)':'🪨 ROBUST'):(th?'⚠ เปราะ (FRAGILE)':'⚠ FRAGILE'))+'</div><div style="font-size:11.5px;color:#8a98b8;margin-top:7px">'+note+'</div></div>';};
...
+'</div><div style="margin-top:10px;font-size:13px;color:#2dd4bf;font-weight:700">→ '+(th?'ยิ่งเลขสูง ยิ่งเชื่อถือได้ · พิสูจน์แบบ exact: ปลอม m จุดพลิก, m−1 ไม่พลิก':'a higher number is a decision you can stand behind · exact: corrupting m flips, m−1 does not')+'</div>';
document.getElementById('flabout').innerHTML=flabShell(col,(th?'ใบรับรองระยะทนการปลอมข้อมูล':'DECISION-BREAKDOWN CERTIFICATE'),(th?'ต้องปลอมข้อมูลกี่จุดผลถึงพลิก — สิ่งที่ LLM ทำไม่ได้':'how much corrupted data until the verdict flips — an LLM cannot'),body,(th?'พิสูจน์แล้ว: ระยะพลิกคำนวณ exact (m พลิก, m−1 ไม่พลิก) 100% · เลขปลอมสูงเกินจริงถูกจับ 100% · เซ็น Ed25519 · ของจริงจาก /breakdown':'proven: exact breakdown (m flips, m−1 holds) 100% · an inflated robustness claim is caught 100% · Ed25519-signed · live from /breakdown'));}

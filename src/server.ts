...
.primeaura{position:absolute;inset:-2px;border-radius:28px;background:conic-gradient(from 0deg,#e11d48,#f43f8e,#a855f7,#6d5cf0,#0ea5b7,#e11d48);filter:blur(11px);opacity:.55;animation:primeSpin 7s linear infinite;z-index:0}
.primeborder{position:absolute;inset:0;border-radius:26px;padding:2px;background:conic-gradient(from 0deg,#e11d48,#f43f8e,#a855f7,#6d5cf0,#0ea5b7,#e11d48);animation:primeSpin 7s linear infinite;-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask-composite:exclude;z-index:2}
.primeinner{position:relative;z-index:3;background:rgba(255,255,255,.94);backdrop-filter:blur(8px);border-radius:24px;padding:24px 26px;overflow:hidden}
.primegem{width:30px;height:30px;border-radius:6px;background:linear-gradient(135deg,#e11d48,#f43f8e 40%,#a855f7);box-shadow:0 6px 18px -4px rgba(225,29,72,.6),inset 0 1px 3px rgba(255,255,255,.7);animation:primeFloat 4s ease-in-out infinite}
.primeglint{background:linear-gradient(110deg,transparent 30%,rgba(255,255,255,.85) 48%,transparent 66%);background-size:220% 100%;-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;animation:primeShimmer 4.5s linear infinite}
@media(prefers-reduced-motion:reduce){.primeaura,.primeborder,.primegem,.primeglint,.primewrap{animation:none}}
...
el.style.display='block';
el.innerHTML='<div class="primewrap" style="box-shadow:0 40px 90px -40px rgba(190,20,80,.7)"><div class="primeaura"></div><div class="primeborder"></div><div class="primeinner"><div style="position:absolute;inset:0;background:radial-gradient(55% 70% at 100% 0%,rgba(225,29,72,.07),transparent 60%),radial-gradient(50% 60% at 0% 100%,rgba(109,92,240,.06),transparent 60%);pointer-events:none"></div><div style="position:relative">'
+'<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px"><span class="primegem"></span><span class="primeglint" style="font-size:16px;font-weight:900;letter-spacing:2px">MELETE PRIME</span><span style="font-size:10.5px;font-weight:800;letter-spacing:.6px;text-transform:uppercase;color:#fff;background:linear-gradient(135deg,#e11d48,#a855f7);padding:3px 9px;border-radius:99px">'+(th?'เพชรแดง · สมองหลัก':'Red Diamond · the brain')+'</span></div>'

...
<div class="hero">
  ...
  <span class="eyebrow" data-i18n="eyebrow">Self-improving systems · runs on your machine</span>
  ...
  <p class="sub" data-i18n="heroSub">Drug formulas · solar tracking · material mixes · battery chemistry — on your machine, never the cloud, every answer signed.</p><p class="sub" style="margin-top:-18px">v${version}</p>
  <div class="cta">
...
function renderPrime(){var j=window.LASTJ;var el=document.getElementById('prime');if(!el)return;var pr=j&&j.prime;if(!pr||!isFinite(pr.processIQ)||pr.grade==='unknown'){el.style.display='none';return;}var th=(LANG==='th');var kc={safety:'#c0392b','feasibility':'#b45309',trust:'#b45309',refine:'#b45309',ship:'#0e9f6e','more-data':'#3b82f6'}[pr.decisive.kind]||'#6d28d9';var gl=th?({'world-class':'ระดับโลก',strong:'แข็งแรง',developing:'กำลังพัฒนา',fragile:'เปราะบาง'}[pr.grade]):pr.grade;var iq=pr.processIQ;var deg=Math.round(iq*3.6);var head=th?({safety:'⚠ ความปลอดภัยมาก่อน',feasibility:'🎯 ต้องเพิ่มตัวแปรใหม่',trust:'⏱ ตรวจสอบความน่าเชื่อก่อน',refine:'🔁 ปรับให้ทนทานก่อน',ship:'✅ พร้อมใช้งานจริง','more-data':'↗ ลองต่ออีกหน่อย'}[pr.decisive.kind]):({safety:'⚠ Safety first',feasibility:'🎯 Add a new lever',trust:'⏱ Verify trust first',refine:'🔁 Make it robust first',ship:'✅ Ready to ship','more-data':'↗ Keep going'}[pr.decisive.kind]);
el.style.display='block';
el.innerHTML='<div style="position:relative;border-radius:24px;padding:2px;background:linear-gradient(135deg,#e11d48,#b91c6b,#6d5cf0);box-shadow:0 30px 70px -34px rgba(190,30,90,.6)"><div style="background:#fff;border-radius:22px;padding:22px 24px;position:relative;overflow:hidden"><div style="position:absolute;inset:0;background:radial-gradient(60% 70% at 100% 0%,rgba(225,29,72,.06),transparent 60%);pointer-events:none"></div><div style="position:relative;display:flex;gap:22px;align-items:center;flex-wrap:wrap"><div style="flex:0 0 auto;width:118px;height:118px;border-radius:50%;background:conic-gradient('+kc+' '+deg+'deg,#f1eef6 0)"><div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center"><div style="width:92px;height:92px;border-radius:50%;background:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:inset 0 0 0 1px #f3eef8"><div style="font-family:ui-monospace,Menlo,monospace;font-size:30px;font-weight:800;color:'+kc+';line-height:1">'+iq+'</div><div style="font-size:9px;letter-spacing:.5px;text-transform:uppercase;color:#9aa0b8;font-weight:700;margin-top:1px">'+(th?'สมอง':'IQ')+'</div></div></div></div><div style="flex:1;min-width:240px"><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><span style="font-size:12px;font-weight:800;letter-spacing:1px;text-transform:uppercase;background:linear-gradient(135deg,#e11d48,#6d5cf0);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent">◆ MELETE PRIME</span><span style="font-size:11px;color:#b91c6b;font-weight:700">'+(th?'สมองหลัก — ฉลาดทุกเรื่องในที่เดียว':'the brain — smart about everything, in one verdict')+'</span></div><div style="font-size:20px;font-weight:800;color:'+kc+';margin:5px 0 2px;letter-spacing:-.3px">'+head+'</div><div style="font-size:13.5px;color:#33344e;line-height:1.6">'+pr.briefing+'</div><div style="font-size:11.5px;color:#9aa0b8;margin-top:7px">'+(th?'รวมทุกมุมมอง (ประสิทธิภาพ·ทนทาน·ปลอดภัย·เพดาน·ความน่าเชื่อ·ปุ่มที่สำคัญ) → จัดลำดับความสำคัญเป็น 1 คำตัดสิน · '+gl:'composes every lens (efficiency·robustness·safety·ceiling·trust·which-knobs) → one prioritized decision · '+gl)+'</div></div></div></div></div>';}
...
function renderBrain(){var j=window.LASTJ;var el=document.getElementById('brain');if(!el)return;var t=j&&j.lineage;if(!t||!t.nodes||t.nodes.length<3||t.root<0){el.style.display='none';return;}var th=(LANG==='th');var W=620,H=360,pad=30;var N=t.nodes;var vmin=Infinity,vmax=-Infinity;for(var i=0;i<N.length;i++){if(N[i].value<vmin)vmin=N[i].value;if(N[i].value>vmax)vmax=N[i].value;}var vr=Math.max(1e-9,vmax-vmin);
var X=function(x){return pad+x*(W-2*pad);},Y=function(y){return H-pad-y*(H-2*pad);};
var lerp=function(t01){var a=[199,195,240],b=[14,165,183];return 'rgb('+Math.round(a[0]+(b[0]-a[0])*t01)+','+Math.round(a[1]+(b[1]-a[1])*t01)+','+Math.round(a[2]+(b[2]-a[2])*t01)+')';};
var fmtv=function(v){return (Math.abs(v)<1?(+v).toFixed(2):(+v).toFixed(1));};
// store screen coords for the interactive click-to-trace sandbox
var SC=N.map(function(nd){return {sx:X(nd.x),sy:Y(nd.y),parent:nd.parent,value:nd.value,i:nd.i};});
window.BRAIN={W:W,H:H,root:t.root,nodes:SC};
// label the trunk edges (highest-scoring) with the real score transition — connects the tree to reality
var byVal=N.map(function(nd,k){return k;}).sort(function(a,b){return N[b].value-N[a].value;});
var labelSet={};for(var k=0;k<Math.min(5,byVal.length);k++)labelSet[byVal[k]]=1;
var dims=(j.space||[]);var obsr=(j.observations||[]);
var edges='',nodes='',labels='';
for(var i=0;i<N.length;i++){var nd=N[i];if(nd.parent!=null){var p=N[nd.parent];var x1=X(nd.x),y1=Y(nd.y),x2=X(p.x),y2=Y(p.y);var mx=(x1+x2)/2,my=(y1+y2)/2;var dx=x2-x1,dy=y2-y1;var len=Math.sqrt(dx*dx+dy*dy)||1;var cx=mx-dy/len*len*0.16,cy=my+dx/len*len*0.16;var tt=(nd.value-vmin)/vr;var sw=(0.6+tt*2.6).toFixed(2);var op=(0.18+tt*0.6).toFixed(2);var dl=(nd.depth*0.06).toFixed(2);edges+='<path d="M'+x1.toFixed(1)+' '+y1.toFixed(1)+' Q'+cx.toFixed(1)+' '+cy.toFixed(1)+' '+x2.toFixed(1)+' '+y2.toFixed(1)+'" fill="none" stroke="'+lerp(tt)+'" stroke-width="'+sw+'" stroke-linecap="round" opacity="'+op+'" pathLength="1" style="stroke-dasharray:1;stroke-dashoffset:1;animation:branchGrow .9s ease forwards;animation-delay:'+dl+'s"/>';if(labelSet[i]&&len>40){var lx=cx,ly=cy-3;labels+='<text x="'+lx.toFixed(1)+'" y="'+ly.toFixed(1)+'" font-size="10" font-weight="700" fill="#4c3fd6" text-anchor="middle" opacity="0" style="font-family:ui-monospace,Menlo,monospace;paint-order:stroke;stroke:#fff;stroke-width:3.2px;stroke-linejoin:round;animation:nodePop .6s ease forwards;animation-delay:'+(+dl+0.7)+'s">'+fmtv(nd.value)+'→'+fmtv(p.value)+'</text>';}}}
// neural pulse: a signal travelling the longest branch INTO the brain core (the optimum)
var deep=0;for(var i=0;i<N.length;i++)if(N[i].depth>N[deep].depth)deep=i;
var pc=[];var pcur=deep,pg=0;while(pcur!=null&&pg++<N.length+1){pc.push(pcur);pcur=N[pcur].parent;}
var pulse='';if(pc.length>2){var pd='';for(var i=0;i<pc.length;i++)pd+=(i?' L':'M')+X(N[pc[i]].x).toFixed(1)+' '+Y(N[pc[i]].y).toFixed(1);pulse='<circle r="3.4" fill="#6d5cf0"><animateMotion dur="2.8s" repeatCount="indefinite" path="'+pd+'"/><animate attributeName="opacity" values="0;1;1;0" dur="2.8s" repeatCount="indefinite"/></circle>';}
for(var i=0;i<N.length;i++){var nd=N[i];var tt=(nd.value-vmin)/vr;var isRoot=(nd.i===t.root);var r=(isRoot?9:(2+tt*4.5)).toFixed(1);var cxx=X(nd.x).toFixed(1),cyy=Y(nd.y).toFixed(1);var dl=(nd.depth*0.06+0.1).toFixed(2);var tip='score '+(+nd.value).toFixed(3);if(obsr[nd.i]&&obsr[nd.i].experiment&&dims.length)tip+=' · '+dims.map(function(d){return d.name+'='+(+obsr[nd.i].experiment[d.name]).toFixed(2);}).join(', ');if(isRoot)nodes+='<circle cx="'+cxx+'" cy="'+cyy+'" r="17" fill="#14b8a6" opacity="0.16"><animate attributeName="r" values="14;20;14" dur="3s" repeatCount="indefinite"/></circle>';nodes+='<circle cx="'+cxx+'" cy="'+cyy+'" r="'+r+'" fill="'+(isRoot?'#0ea5b7':lerp(tt))+'" opacity="'+(0.45+tt*0.55).toFixed(2)+'" style="opacity:0;animation:nodePop .5s ease forwards;animation-delay:'+dl+'s"><title>'+(isRoot?'★ best · ':'')+tip+'</title></circle>';}
el.style.display='block';
el.innerHTML='<div style="background:rgba(255,255,255,.82);backdrop-filter:blur(12px);border:1px solid #ece8fb;border-radius:20px;padding:20px 22px;box-shadow:0 26px 58px -34px rgba(70,55,160,.5)"><div style="display:flex;align-items:baseline;gap:9px;flex-wrap:wrap;margin-bottom:3px"><span style="font-size:13px;font-weight:800;letter-spacing:.4px;text-transform:uppercase;color:#6d28d9">🧠 '+(th?'สมองการค้นพบ':'Discovery brain')+'</span><span style="font-size:12.5px;color:#8890a8">'+(th?'การค้นหาแตกกิ่งเหมือนรากสมอง — ทุกกิ่งไหลขึ้นสู่จุดที่ดีที่สุด':'the search as branching roots — every branch flows uphill to the best point')+'</span></div><div style="font-size:12px;color:#6d28d9;font-weight:600;margin-bottom:4px">👆 '+(th?'คลิกจุดไหนก็ได้ดูเส้นทางไปจุดดีสุด · ชี้ค้างที่จุดเพื่อดูสูตร+คะแนน':'Click anywhere to trace the path to the best · hover a node to see its recipe + score')+'</div><svg viewBox="0 0 '+W+' '+H+'" style="width:100%;height:auto;display:block;cursor:crosshair" onclick="gBrainClick(event)">'+edges+'<g id="brainpath"></g>'+pulse+nodes+labels+'</svg><div style="font-size:11.5px;color:#9aa0b8;margin-top:6px">'+(th?'จุดสว่าง/ใหญ่ = คะแนนสูง · 🟢 แกนกลางเรืองแสง = จุดที่ดีที่สุด · ป้ายตัวเลข = คะแนนก่อน→หลัง · '+N.length+' การทดลอง':'brighter / bigger = higher score · 🟢 glowing core = the optimum · labels = score before→after · '+N.length+' experiments')+'</div></div>';}
function gBrainClick(evt){var B=window.BRAIN;if(!B)return;var svg=evt.currentTarget;var r=svg.getBoundingClientRect();var mx=(evt.clientX-r.left)/r.width*B.W,my=(evt.clientY-r.top)/r.height*B.H;var bi=-1,bd=1e18;for(var i=0;i<B.nodes.length;i++){var dd=(B.nodes[i].sx-mx)*(B.nodes[i].sx-mx)+(B.nodes[i].sy-my)*(B.nodes[i].sy-my);if(dd<bd){bd=dd;bi=i;}}if(bi<0)return;var chain=[];var cur=bi,g=0;while(cur!=null&&g++<B.nodes.length+1){chain.push(cur);cur=B.nodes[cur].parent;}var d='M'+B.nodes[chain[0]].sx.toFixed(1)+' '+B.nodes[chain[0]].sy.toFixed(1);for(var i=1;i<chain.length;i++)d+=' L'+B.nodes[chain[i]].sx.toFixed(1)+' '+B.nodes[chain[i]].sy.toFixed(1);var s=B.nodes[chain[0]],e=B.nodes[chain[chain.length-1]];var g2=document.getElementById('brainpath');if(!g2)return;g2.innerHTML='<path d="'+d+'" fill="none" stroke="#6d5cf0" stroke-width="3.2" stroke-linecap="round" stroke-linejoin="round" opacity="0.95" pathLength="1" style="stroke-dasharray:1;stroke-dashoffset:1;animation:branchGrow 1.1s ease forwards"/><circle cx="'+s.sx.toFixed(1)+'" cy="'+s.sy.toFixed(1)+'" r="6" fill="#6d5cf0"><animate attributeName="r" values="5;9;5" dur="1.4s" repeatCount="indefinite"/></circle><circle cx="'+e.sx.toFixed(1)+'" cy="'+e.sy.toFixed(1)+'" r="8" fill="none" stroke="#0ea5b7" stroke-width="2.5"><animate attributeName="r" values="8;13;8" dur="1.6s" repeatCount="indefinite"/></circle>';}

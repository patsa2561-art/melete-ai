...
+'</div><div style="margin-top:10px;font-size:13px;color:#2dd4bf;font-weight:700">→ '+(th?'BH ตัดของที่อยู่ใต้ขีด '+j.bh.threshold+' ทิ้ง — รายงานน้อยลงแต่เชื่อถือได้ · ตรวจซ้ำ offline ได้':'BH cut everything above the '+j.bh.threshold+' cutoff — fewer findings, but trustworthy · re-checkable offline')+'</div>';
document.getElementById('flabout').innerHTML=flabShell(col,(th?'ใบรับรองคุมการค้นพบเท็จ (FDR)':'FALSE-DISCOVERY CERTIFICATE'),(th?'รายงานหลายอย่างพร้อมกันโดยคุมของปลอม — สิ่งที่ LLM ทำไม่ได้':'report many findings while controlling the false ones — an LLM cannot'),body,(th?'พิสูจน์แล้ว: BH คุม FDP จริง ≤ q (วัด 7.6% ≤ 10%) · naive พุ่งเป็น 13% · ยังจับผลจริงได้ 89% · ปลอมเพิ่ม discovery ถูกจับ · เซ็น Ed25519':'proven: BH holds realized FDP ≤ q (measured 7.6% ≤ 10%) · naive inflates to 13% · still recovers 89% of real effects · a forged discovery is caught · Ed25519-signed'));}

...
document.getElementById('flabout').innerHTML=flabShell(col,(th?'MCP — มิดเดิลแวร์แห่งความจริง':'MCP — THE TRUST MIDDLEWARE'),(th?'AI agent ทุกค่ายเรียก Melete แล้วได้คำตอบที่ตรวจสอบเองได้':'any AI agent calls Melete and gets an answer it can verify itself'),body,(th?'นี่คือกลยุทธ์ unicorn: ไม่แข่งกับ editor — เป็นชั้นความจำ/ความจริงที่ทุก agent เสียบใช้ (Claude/GPT/Gemini) · ทุกผลเซ็น Ed25519 · ตรวจซ้ำ offline · ของจริงจาก /mcp':'the unicorn play: do not compete with editors — be the memory/trust layer every agent plugs into (Claude/GPT/Gemini) · every result Ed25519-signed · re-checkable offline · live from /mcp (stdio: melete-mcp)'));}

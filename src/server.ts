...
+'</div><div style="margin-top:10px;font-size:13px;color:#2dd4bf;font-weight:700">→ '+(th?'ขอบล่างที่แก้แล้ว ≤ ค่าจริง = '+(j.boundIsValid?'ใช่ ✓ (ขอบเขตถูกต้อง)':'—')+' · ยิ่งค้นห N มาก โทษยิ่งมาก':'corrected bound ≤ the truth = '+(j.boundIsValid?'yes ✓ (a valid bound)':'—')+' · the more settings you search, the larger the discount')+'</div>';
document.getElementById('flabout').innerHTML=flabShell(col,(th?'ใบรับรองอคติจากการเลือก (Winner Curse)':'SELECTION-BIAS CERTIFICATE'),(th?'แก้ตัวเลขที่เฟ้อจากการเลือก max — สิ่งที่ LLM ทำไม่ได้':'de-bias the inflated best-of-N — an LLM cannot'),body,(th?'พิสูจน์แล้ว: ขอบล่างถูกต้อง ≥97.5% (วัด 99.5%) · เลข naive เฟ้อจริง 90% · โทษโตตาม N (monotone) · ปลอมให้โทษน้อยลงถูกจับ · เซ็น Ed25519 · ของจริงจาก /selection':'proven: lower bound valid ≥97.5% (measured 99.5%) · naive overstates 90% · discount monotone in N · a hidden-curse forgery is caught · Ed25519-signed · live from /selection'));}

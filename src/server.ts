...
var labelSet={};for(var k=0;k<Math.min(7,byVal.length);k++)labelSet[byVal[k]]=1;
var edges='',nodes='',labels='';
for(var i=0;i<N.length;i++){var nd=N[i];if(nd.parent!=null){var p=N[nd.parent];var x1=X(nd.x),y1=Y(nd.y),x2=X(p.x),y2=Y(p.y);var mx=(x1+x2)/2,my=(y1+y2)/2;var dx=x2-x1,dy=y2-y1;var len=Math.sqrt(dx*dx+dy*dy)||1;var cx=mx-dy/len*len*0.16,cy=my+dx/len*len*0.16;var tt=(nd.value-vmin)/vr;var sw=(0.6+tt*2.6).toFixed(2);var op=(0.18+tt*0.6).toFixed(2);var dl=(nd.depth*0.06).toFixed(2);edges+='<path d="M'+x1.toFixed(1)+' '+y1.toFixed(1)+' Q'+cx.toFixed(1)+' '+cy.toFixed(1)+' '+x2.toFixed(1)+' '+y2.toFixed(1)+'" fill="none" stroke="'+lerp(tt)+'" stroke-width="'+sw+'" stroke-linecap="round" opacity="'+op+'" pathLength="1" style="stroke-dasharray:1;stroke-dashoffset:1;animation:branchGrow .9s ease forwards;animation-delay:'+dl+'s"/>';if(labelSet[i]&&len>34){var lx=(mx+cx)/2,ly=(my+cy)/2;labels+='<text x="'+lx.toFixed(1)+'" y="'+ly.toFixed(1)+'" font-size="8.5" fill="#7a83a8" text-anchor="middle" opacity="0" style="font-family:ui-monospace,Menlo,monospace;animation:nodePop .6s ease forwards;animation-delay:'+(+dl+0.7)+'s">'+fmtv(nd.value)+'→'+fmtv(p.value)+'</text>';}}}
for(var i=0;i<N.length;i++){var nd=N[i];var tt=(nd.value-vmin)/vr;var isRoot=(nd.i===t.root);var r=(isRoot?9:(2+tt*4.5)).toFixed(1);var cxx=X(nd.x).toFixed(1),cyy=Y(nd.y).toFixed(1);var dl=(nd.depth*0.06+0.1).toFixed(2);if(isRoot)nodes+='<circle cx="'+cxx+'" cy="'+cyy+'" r="17" fill="#14b8a6" opacity="0.16"><animate attributeName="r" values="14;20;14" dur="3s" repeatCount="indefinite"/></circle>';nodes+='<circle cx="'+cxx+'" cy="'+cyy+'" r="'+r+'" fill="'+(isRoot?'#0ea5b7':lerp(tt))+'" opacity="'+(0.45+tt*0.55).toFixed(2)+'" style="opacity:0;animation:nodePop .5s ease forwards;animation-delay:'+dl+'s"/>';}
el.style.display='block';
el.innerHTML='<div style="background:rgba(255,255,255,.82);backdrop-filter:blur(12px);border:1px solid #ece8fb;border-radius:20px;padding:20px 22px;box-shadow:0 26px 58px -34px rgba(70,55,160,.5)"><div style="display:flex;align-items:baseline;gap:9px;flex-wrap:wrap;margin-bottom:3px"><span style="font-size:13px;font-weight:800;letter-spacing:.4px;text-transform:uppercase;color:#6d28d9">🧠 '+(th?'สมองการค้นพบ':'Discovery brain')+'</span><span style="font-size:12.5px;color:#8890a8">'+(th?'การค้นหาแตกกิ่งเหมือนรากสมอง — ทุกกิ่งไหลขึ้นสู่จุดที่ดีที่สุด':'the search as branching roots — every branch flows uphill to the best point')+'</span></div><div style="font-size:12px;color:#6d28d9;font-weight:600;margin-bottom:4px">👆 '+(th?'คลิกจุดไหนก็ได้ — ดูเส้นทางจาก "จุดเริ่มของคุณ" วิ่งไปยังจุดที่ดีที่สุด':'Click anywhere — watch the path from "your start" race to the best point')+'</div><svg viewBox="0 0 '+W+' '+H+'" style="width:100%;height:auto;display:block;cursor:crosshair" onclick="gBrainClick(event)">'+edges+'<g id="brainpath"></g>'+labels+nodes+'</svg><div style="font-size:11.5px;color:#9aa0b8;margin-top:6px">'+(th?'จุดสว่าง/ใหญ่ = คะแนนสูง · 🟢 แกนกลางเรืองแสง = จุดที่ดีที่สุด · ป้ายตัวเลข = คะแนนก่อน→หลัง · '+N.length+' การทดลอง':'brighter / bigger = higher score · 🟢 glowing core = the optimum · labels = score before→after · '+N.length+' experiments')+'</div></div>';}

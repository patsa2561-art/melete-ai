...
fetch(url,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json();}).then(function(j){if(j.error){if(out)out.innerHTML='<div style="color:#c33;font-size:13px">⚠ '+j.error+'</div>';return;}if(which==='noise')flabNoise(j);else if(which==='mixed')flabMixed(j);else if(which==='null')flabNull(j);else flabProv(j);}).catch(function(e){if(out)out.innerHTML='<div style="color:#c33;font-size:13px">⚠ '+e.message+'</div>';});}
function flabNull(j){var th=(LANG==='th');var col='#9aa6c8';

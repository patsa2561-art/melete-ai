...
<section id="flab" style="margin-top:30px">
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:14px;margin-top:18px">
    ...
    <div class="galcard" onclick="gImp()" style="--gc:#fb923c"><div class="gtitle" data-i18n="flabImpT">📜 Proof of Improvement</div><div class="gknobs" data-i18n="flabImpD">Justify a change: certify recipe B beats your current setting A by ≥Δ — noise-aware, a 97.5% lower bound, signed. Refuses when it's within noise.</div><div class="grun" data-i18n="flabRun">▶ run live</div></div>
  </div>
...
var go=function(sc){return fetch('/improvement',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({scenario:sc,seed:7})}).then(function(r){return r.json();});};
Promise.all([go('real'),go('same')]).then(function(arr){if(arr[0].error||arr[1].error){if(out)out.innerHTML='<div style="color:#c33;font-size:13px">⚠ '+(arr[0].error||arr[1].error)+'</div>';return;}window.__lastImp=arr;renderImp(arr[0],arr[1]);}).catch(function(e){if(out)out.innerHTML='<div style="color:#c33;font-size:13px">⚠ '+e.message+'</div>';});}
function renderImp(real,same){var th=(LANG==='th');var col='#fb923c';
var panel=function(title,j,note){var win=(j.verdict==='IMPROVEMENT');var c=win?'#34d399':'#fbbf24';return '<div style="flex:1;min-width:220px;background:rgba(255,255,255,.04);border:1px solid '+c+'55;border-radius:14px;padding:14px 16px"><div style="font-size:11px;letter-spacing:.5px;text-transform:uppercase;color:#9fb0d0;margin-bottom:6px">'+title+'</div><div style="font-size:20px;font-weight:800;color:'+c+';text-shadow:0 0 14px '+c+'66">'+(win?(th?'✓ ดีขึ้นแน่นอน':'✓ IMPROVEMENT'):(th?'— สรุปไม่ได้':'— INCONCLUSIVE'))+'</div><div style="font-size:12px;color:#cdd6ee;margin-top:6px">'+(th?'วัดได้ +':'observed +')+j.observedGain+' · '+(win?(th?'รับประกัน ≥ +':'guaranteed ≥ +')+j.certifiedGain+' (97.5%)':(th?'อยู่ในช่วง noise':'within the noise'))+'</div><div style="font-size:11.5px;color:#8a98b8;margin-top:7px">'+note+'</div></div>';};
var body='<div style="font-size:13px;color:#cdd6ee;margin:2px 2px 12px">'+(th?'วัดซ้ำ A กับ B ภายใต้ noise แล้วออกขอบล่าง 97.5% ของส่วนต่าง — รับรอง "ดีขึ้น" เฉพาะเมื่อพิสูจน์ได้เกิน noise':'replicate-measure A vs B under noise, then a 97.5% lower bound on the gain — certifies "improvement" ONLY when it provably clears the noise')+'</div><div style="display:flex;gap:12px;flex-wrap:wrap">'
+panel((th?'B ดีกว่าจริง':'B is genuinely better'),real,(th?'รับรอง + ออกใบเซ็นให้ใช้ตัดสินใจเปลี่ยน':'certified → a signed basis to switch'))
+panel((th?'A กับ B เท่ากัน':'A and B are equal'),same,(th?'ปฏิเสธอย่างซื่อสัตย์ — ไม่หลอกว่าดีขึ้น':'honestly refuses — no fake "improvement"'))
+'</div>';
document.getElementById('flabout').innerHTML=flabShell(col,(th?'ใบพิสูจน์การปรับปรุง':'PROOF OF IMPROVEMENT'),(th?'เปลี่ยนเพราะดีขึ้นจริง ไม่ใช่เพราะ noise':'switch on a real gain, not noise'),body,(th?'พิสูจน์แล้ว: ขอบล่างถูกต้อง ≥97.5% · หลอกบวก ≤2.5% · เซ็น Ed25519 · ของจริงจาก /improvement':'proven: lower bound valid ≥97.5% · false-positive ≤2.5% · Ed25519-signed · live from /improvement'));}
...
function setLang(l){LANG=l;try{localStorage.setItem('mlang',l);}catch(e){}
 ...
 try{if(window.__lastImp&&typeof renderImp==='function')renderImp(window.__lastImp[0],window.__lastImp[1]);}catch(e){}

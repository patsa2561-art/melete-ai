...
+'</div><div style="margin-top:10px;font-size:13px;color:#2dd4bf;font-weight:700">→ '+(th?'การันตีไม่ขึ้นกับการแจกแจง · ช่วง ŷ±q = ['+j.conformal.interval[0]+', '+j.conformal.interval[1]+'] · ตรวจซ้ำ '+(j.verified?'✓':'✕'):'a guarantee that needs no distribution · ŷ±q = ['+j.conformal.interval[0]+', '+j.conformal.interval[1]+'] · verified '+(j.verified?'✓':'✕'))+'</div>';
document.getElementById('flabout').innerHTML=flabShell(col,(th?'ใบรับรอง Conformal Prediction':'CONFORMAL-PREDICTION CERTIFICATE'),(th?'ช่วงทำนายที่การันตี coverage โดยไม่สมมติการแจกแจง — สิ่งที่ LLM ทำไม่ได้':'a prediction interval with guaranteed coverage, no assumption — an LLM cannot'),body,(th?'พิสูจน์แล้ว: coverage ≥1−α ทุกการแจกแจง (normal/heavy/skewed ~90%) · finite-sample exact (n=20 ได้ 90.4%) · บน skewed แคบกว่า Gaussian 22% · ปลอมแคบเกินถูกจับ · เซ็น Ed25519':'proven: coverage ≥1−α across every law (normal/heavy/skewed ~90%) · finite-sample exact (n=20 holds 90.4%) · 22% tighter than Gaussian on skew · an over-confident interval is caught · Ed25519-signed'));}

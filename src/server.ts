...
export function docsPage(version: string | number) { return "<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Melete API — connect your process</title>\n<style>body{font:15px/1.65 ui-sans-serif,system-ui,-apple-system,\"Segoe UI\",sans-serif;max-width:780px;margin:0 auto;padding:34px 20px;color:#1a1b30}h1{font-size:30px;margin:8px 0 4px;letter-spacing:-1px}h2{font-size:13px;text-transform:uppercase;letter-spacing:.5px;color:#6d28d9;margin:30px 0 8px}code{font-family:ui-monospace,Menlo,monospace;background:#f3f1ff;color:#4338ca;padding:1px 5px;border-radius:5px;font-size:13px}pre{background:#1a1b30;color:#e7e7f5;padding:15px;border-radius:11px;overflow:auto;font-size:12.5px;line-height:1.5}pre code{background:none;color:inherit;padding:0}.f{border-collapse:collapse;width:100%;font-size:14px;margin:8px 0}.f td{border-bottom:1px solid #eee;padding:8px 6px;vertical-align:top}.f td:first-child{font-family:ui-monospace,monospace;color:#6d28d9;white-space:nowrap;font-size:13px}a{color:#5b53e8;font-weight:600;text-decoration:none}a:hover{text-decoration:underline}.muted{color:#8890a8;font-size:13px}.card{border:1px solid #e7e8f0;border-radius:14px;padding:18px 18px 6px;margin:14px 0}.ret{display:inline-block;background:#ecfdf5;color:#0e7a4f;font-size:11px;font-weight:700;padding:2px 8px;border-radius:6px;text-transform:uppercase;letter-spacing:.3px}</style>\n</head><body>\n<a href=\"/\">&larr; Melete</a>\n<h1>Melete API</h1>\n<p class=\"muted\">Connect your real process. Base URL <code>https://melete.mneme-ai.space</code> &middot; all JSON &middot; no auth on the demo.</p>\n<p><b>The loop:</b> POST your results so far &rarr; get the next experiment to try &rarr; run it in your system &rarr; measure a score &rarr; POST again with the new result &rarr; repeat until it converges. <b>You</b> run the experiment; <b>Melete</b> decides what to try next.</p>\n<h2>POST /next &mdash; the guided loop</h2><div class=\"card\"><table class=\"f\">\n<tr><td>space</td><td>what you can change: <code>[{name, type:\"real\"|\"int\", min, max}]</code></td></tr>\n<tr><td>observations</td><td>everything measured so far: <code>[{experiment:{name:value}, value:number}]</code> &mdash; empty <code>[]</code> on the first call</td></tr>\n<tr><td>goal</td><td><code>\"maximize\"</code> or <code>\"minimize\"</code></td></tr>\n<tr><td>costPerExperiment</td><td><i>optional</i> &mdash; cost per experiment; enables the money stop-advice</td></tr>\n</table><p><span class=\"ret\">returns</span> <code>{ next, best, advice:{recommendation:\"CONTINUE\"|\"STOP\"} }</code> &mdash; set your system to <code>next</code>, measure, then POST again with it appended to <code>observations</code>.</p>\n<pre><code>curl -s https://melete.mneme-ai.space/next -H \"content-type: application/json\" \\\n  -d '{\"space\":[{\"name\":\"pH\",\"type\":\"real\",\"min\":3,\"max\":9}],\"observations\":[{\"experiment\":{\"pH\":6},\"value\":7.2}],\"goal\":\"maximize\"}'</code></pre></div>\n<h2>POST /next-multi &mdash; multi-objective (Pareto)</h2><div class=\"card\"><p>Like <code>/next</code>, but <code>goals</code> has one entry per objective and each observation carries <code>values[]</code>:</p>\n<pre><code>{\"space\":[...], \"goals\":[{\"name\":\"yield\",\"goal\":\"maximize\"},{\"name\":\"cost\",\"goal\":\"minimize\"}],\n \"observations\":[{\"experiment\":{...}, \"values\":[90, 40]}]}</code></pre>\n<p><span class=\"ret\">returns</span> <code>{ next, paretoFront:[ ...best trade-offs... ], paretoSize }</code></p></div>\n<h2>POST /discover &mdash; one-shot (you supply a formula)</h2><div class=\"card\"><p>For automatable objectives: pass an <code>objective</code> expression in your variable names &mdash; Melete runs the whole loop and returns the best + a signed trace + a Proof of Optimization.</p>\n<pre><code>{\"space\":[{\"name\":\"x\",\"type\":\"real\",\"min\":-5,\"max\":5}], \"objective\":\"-(x-2)**2\", \"budget\":40, \"goal\":\"maximize\"}</code></pre>\n<p><span class=\"ret\">returns</span> <code>{ best, evaluations, frontier, certificate, poopt, trace, verify }</code></p></div>\n<h2>POST /poopt/verify &mdash; verify a certificate offline</h2><div class=\"card\"><p>POST a downloaded <code>proof-of-optimization.json</code> &rarr; <code>{ ok, reason, efficiencyPct }</code>. Zero-server option: <code>npm i -g melete-ai</code> then <code>melete poopt cert.json</code>.</p></div>\n<h2>Self-hosted &amp; air-gapped</h2><div class=\"card\"><p><code>npm i -g melete-ai</code> &middot; run <code>melete-server</code> on your own machine &mdash; data never leaves. Or call the library directly:</p><pre><code>import { proposeNext, proposeNextMulti } from \"melete-ai\"</code></pre></div>\n<p class=\"muted\" style=\"margin-top:24px\">Melete v__VER__ &middot; MIT &middot; <a href=\"/\">home</a></p>\n</body></html>".split("__VER__").join(String(version)); }

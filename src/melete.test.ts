...
describe("gauntlets (every module = 100)", () => {
  ...
  it("aggregate meleteGauntlet = 100 over all 46 modules", async () => {
    const g = await meleteGauntlet();
    expect(g.score).toBe(100);
    expect(g.modules.map((m) => m.name).sort()).toEqual(["achievability", "aegis", "arms", "batch", "bench", "certify", "cliff", "confidence", "constrained", "cortex", "costaware", "drift", "efficiency", "engine", "federated", "frontier", "guardian", "interaction", "interactive", "inverse", "ipshield", "lineage", "metabrain", "multiobjective", "noise", "oracle", "poopt", "portfolio", "prescription", "prime", "rashomon", "reliability", "replay", "replicate", "resonance", "sensitivity", "server", "shape", "sloppiness", "sovereign", "space", "surprise", "territory", "trace", "transfer", "twin"]);
  });
});

...
describe("gauntlets (every module = 100)", () => {
  ...
  it("aggregate meleteGauntlet = 100 over all 58 modules", async () => {
    const g = await meleteGauntlet();
    expect(g.score).toBe(100);
    expect(g.modules.map((m) => m.name).sort()).toEqual(["achievability", "aegis", "arms", "batch", "bench", "causal", "certify", "cliff", "confidence", "constrained", "cortex", "costaware", "drift", "efficiency", "engine", "federated", "frontier", "guardian", "honestsearch", "improvement", "interaction", "interactive", "inverse", "ipshield", "journalist", "lineage", "metabrain", "mixedspace", "multiobjective", "noise", "noiserobust", "nullengine", "oracle", "poopt", "portfolio", "prereg", "prescription", "prime", "provenance", "rashomon", "reliability", "replay", "replicate", "resonance", "sensitivity", "server", "shape", "sloppiness", "sovereign", "space", "summit", "surprise", "territory", "tolerance", "trace", "transfer", "trustcert", "twin"]);
  });
});

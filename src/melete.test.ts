...
describe("gauntlets (every module = 100)", () => {
  ...
  it("🌐 DRO v2 — distribution-shift robustness + confidence mode (DRO≡CI, Duchi-Namkoong): valid+tight χ²-ball bound, ranks robust, calibrated (1−α) LCB on the true mean, signed", () => expect(droGauntlet().score).toBe(100));
  it("⚖️ FAIRNESS v2 — group + INTERSECTIONAL fairness: catches XOR-gerrymander bias the marginals miss, no false alarm ≤ α, names the intersection, signed", () => expect(fairnessGauntlet().score).toBe(100));
  ...
  it("✍️ CONSENT CERTIFICATE — two-party GDPR consent: scoped grant + use adjudication (purpose/field/expiry/revocation), subject can prove a violation, forged ALLOW caught, signed", () => expect(consentGauntlet().score).toBe(100));
  it("🎫 TRUST PASSPORT — composes many certs into one signed bundle, one-call offline verify, bad/swapped member caught, merkle order-independent, two-party receipt", () => expect(passportGauntlet().score).toBe(100));
  it("🧬 MODEL SUPPLY-CHAIN (AIBOM) — multi-party (≥3) signed lineage: each step signed by its party, tamper/reorder/impersonation/broken-provenance caught", () => expect(aibomGauntlet().score).toBe(100));
  it("🚫 REVOCATION REGISTRY — CRL/OCSP for AI certs: GOOD/REVOKED, time-aware (past reliance stays valid), authority-signed, tamper/forge/un-revoke caught", () => expect(revocationGauntlet().score).toBe(100));
  it("💾 DURABLE PUBLIC LOG — the transparency log survives a restart: identical root + key, inclusion/consistency proofs still verify, tamper of history detected", () => expect(durableGauntlet().score).toBe(100));
  it("✅ LIVE TRUST REPORT — one signed answer 'trustworthy RIGHT NOW?': every member verifies ∧ un-revoked (time-aware) ∧ logged → TRUSTED-NOW, else NOT-TRUSTED-NOW naming the member; forged TRUSTED-NOW caught", () => expect(trustReportGauntlet().score).toBe(100));
  it("🏛️ CHAIN OF TRUST — verifiable authority delegation (AI CA): from one pinned root, an issuer's claim is authorized only if every link is signed, in-scope (kind/namespace), in-time, within path budget, and never broadens; wrong-root/out-of-scope/expired/over-delegation/forgery caught", () => expect(authorityGauntlet().score).toBe(100));
  it("🛰️ WITNESS NETWORK — split-view immunity via independent witness co-signing: quorum-accept, forged/duplicate ignored, append-only refusal, split-view detected", () => expect(witnessGauntlet().score).toBe(100));
  it("🌍 AI TRANSPARENCY LOG — Certificate-Transparency-for-AI: inclusion + append-only consistency proofs, rewrite-of-history & split-view caught, signed tree heads (RFC 6962)", () => expect(translogGauntlet().score).toBe(100));
  it("🎟️ PROOF-CARRYING ANSWERS — per-answer runtime trust tag: in-scope→TRUSTED, out-of-scope flagged with witness, low-confidence→NEEDS-REVIEW, forgery caught, signed", () => expect(pcaGauntlet().score).toBe(100));
  it("🕵️ PRIVATE AUDIT PROOF — prove a model claim over a huge private dataset revealing only a tiny Fiat-Shamir sample; honest accept, cheater caught, no cherry-pick, Merkle-bound, signed", () => expect(spotcheckGauntlet().score).toBe(100));
  ...
  it("aggregate meleteGauntlet = 100 over all 87 modules", async () => {
    const g = await meleteGauntlet();
    expect(g.score).toBe(100);
    expect(g.modules.map((m) => m.name).sort()).toEqual(["achievability", "aegis", "aibom", "anytime", "arms", "authority", "batch", "bench", "breakdown", "calibration", "causal", "certify", "cliff", "confidence", "conformal", "consent", "constrained", "cortex", "costaware", "design", "drift", "dro", "durable", "efficiency", "engine", "fairness", "fdr", "federated", "frontier", "guardian", "honestsearch", "improvement", "interaction", "interactive", "inverse", "ipshield", "journalist", "lineage", "mcp", "metabrain", "mixedspace", "multiobjective", "noise", "noiserobust", "nullengine", "oracle", "passport", "pca", "poopt", "portfolio", "prereg", "prescription", "prime", "privacy", "provenance", "rashomon", "receipt", "reliability", "replay", "replicate", "resonance", "revocation", "sensitivity", "server", "shape", "shapley", "sla", "sloppiness", "sovereign", "space", "spotcheck", "subgroup", "summit", "support", "surprise", "swarm", "territory", "tolerance", "trace", "transfer", "translog", "trustcert", "trustreport", "twin", "unlearning", "winnerscurse", "witness"]);
  });
});

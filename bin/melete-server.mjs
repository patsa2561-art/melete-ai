...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && path === "/discover") {
      ...
      return json(res, 200, { best, evaluations: totalEvals, converged: sig.result.converged, engine: sig.engine, reliable, goal, dims, space: space.dims, observations: obsOut, armStats: sig.result.armStats ?? null, surface, path, frontier, certificate, baseline, poopt, sensitivity, noise, interactions, coverage, drift, efficiency, prescription, lineage, sloppiness, cliffs, prime, trace: sig.trace, verify: M.verifyTrace(sig.trace).ok });
    }

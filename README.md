...
| 🌐 **Distribution-Shift (DRO)** *(new)* | every optimizer reports the value it measured on the data it **saw** — but deployment data drifts (the customer mix, the traffic, the population). A setting that looks best on the nominal data can collapse under a modest shift. This certifies the **worst-case mean over every distribution within a χ²-divergence ball** of radius ρ — `V = mean − √(ρ·Var)`, the exact Cauchy-Schwarz-tight bound — guaranteeing "**under any shift up to χ² ≤ ρ, the value is provably ≥ V**", and signs it. AEGIS guards input wobble and Tolerance parameter wobble; this guards the **data distribution** itself, so a high-mean-but-fragile setting is correctly out-ranked by a robust one. *(over 24,000 random reweightings inside the ball none ever beat the certified worst case — a valid bound — and the aligned adversary achieves it exactly; the value is monotone in ρ and recovers the mean at ρ=0; a fragile high-variance setting collapses under shift while a robust one holds)* |

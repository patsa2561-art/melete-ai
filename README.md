...
> **66 independently-verified modules.** Every claim below is a check you can re-run: `npx melete-ai gauntlet`.
...
| 📏 **Conformal Prediction** *(new)* | "ŷ ± 1.96σ" assumes Gaussian noise — wrong on real skewed/heavy-tailed data. **Split-conformal** wraps *any* predictor with a **distribution-free** interval whose coverage is **guaranteed ≥ 1−α, finite-sample exact** (by exchangeability, no assumption). *(coverage lands at the target across normal/heavy/skewed residuals — spread 0.2pp vs the Gaussian's 2.8pp; on skewed data it's 22% tighter than the over-covering Gaussian; exact even at n=20)* |
...
`npm i melete-ai` · CLI `npx melete-ai …` · HTTP `https://melete.mneme-ai.space` — `/next` `/discover` `/trust-certificate` `/stability` `/honest-search` `/tolerance` `/improvement` `/prereg` `/breakdown` `/selection` `/support` `/fdr` `/anytime` `/swarm` `/conformal` `/mcp` `/verify`

...
> **67 independently-verified modules.** Every claim below is a check you can re-run: `npx melete-ai gauntlet`.
...
| 👥 **Subgroup Validity** *(new)* | "B beats A +3% overall" can hide a segment B actively **harms** (Simpson's paradox). It tests the effect in **every subgroup** with Bonferroni multiplicity control — declares **UNIFORM-IMPROVEMENT** only if all segments improve, flags **HARMED-SUBGROUP** and **names** the hurt one, and raises a flag when the pooled average is misleading. *(detects + names a harmed segment 100%; the pooled test says "improvement" while a segment is harmed 99% — the trap; false-harm ≤ α)* |
...
`npm i melete-ai` · CLI `npx melete-ai …` · HTTP `https://melete.mneme-ai.space` — `/next` `/discover` `/trust-certificate` `/stability` `/honest-search` `/tolerance` `/improvement` `/prereg` `/breakdown` `/selection` `/support` `/fdr` `/anytime` `/swarm` `/conformal` `/subgroup` `/mcp` `/verify`

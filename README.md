...
| 🔒 **Privacy** *(new)* | sovereignty keeps your raw data home — but the moment you **share an aggregate** (a federated mean, a published stat, a pooled gradient) you can leak the individuals in it (membership inference). This certifies the release is **(ε,δ)-differentially private** via the **tight analytic Gaussian** (Balle-Wang) — the *minimum* noise — reveals only the noised value, signs it, and a **composition ledger** refuses an over-budget release. The dishonest failure it catches: **under-noising** while claiming a small ε. *(the optimal membership-inference attack on a certified release stays inside the (ε,δ) privacy region; a release with 1/5 the noise leaks far outside it and is rejected on re-derivation; calibration is tight — achieved δ = target δ; the ledger refuses the release that would overspend the budget)* |
...
`npm i melete-ai` · CLI `npx melete-ai …` · HTTP `https://melete.mneme-ai.space` — `/next` `/discover` `/trust-certificate` `/stability` `/honest-search` `/tolerance` `/improvement` `/prereg` `/breakdown` `/selection` `/support` `/fdr` `/anytime` `/swarm` `/conformal` `/subgroup` `/calibration` `/privacy` `/mcp` `/verify`

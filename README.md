...
| 🗑 **Unlearning** *(new)* | the **"right to be forgotten"** with proof. When a user asks to be deleted, a provider's cheapest move is to do nothing and claim it's done. This deletes a record from a **ridge model EXACTLY** via a **Sherman-Morrison rank-1 downdate** — O(d²), touching only that record's own contribution, **never the other n−1 rows** — proves the served model equals one **retrained from scratch** without the record, reports the record's **influence** + the **residual influence** left behind (must be ~0), and signs it. An auditor re-derives the deletion from the **Gram matrix alone** (never the raw rows). The dishonest failure it catches: **fake / partial deletion**. *(the downdate equals full retraining to ~1e-15; a provider that secretly keeps the record is flagged RESIDUAL-INFLUENCE orders of magnitude above tolerance and a forged "DELETED" cert is rejected; deletes without retraining)* |
...
`npm i melete-ai` · CLI `npx melete-ai …` · HTTP `https://melete.mneme-ai.space` — `/next` `/discover` `/trust-certificate` `/stability` `/honest-search` `/tolerance` `/improvement` `/prereg` `/breakdown` `/selection` `/support` `/fdr` `/anytime` `/swarm` `/conformal` `/subgroup` `/calibration` `/privacy` `/unlearning` `/mcp` `/verify`

...
| 🎯 **Calibration** *(new)* | when a model/agent says "90% sure", is it right ~90% of the time? **Spiegelhalter's Z** + **ECE** test whether the stated confidence is trustworthy, **name** the over/under-confidence, and **recalibrate** it. The trust check for any agent that emits a probability. *(a calibrated model is falsely flagged ≤ α; over-confidence detected 100% with direction named 100%; histogram recalibration cuts ECE 10.2%→6.4%)* |
...
`npm i melete-ai` · CLI `npx melete-ai …` · HTTP `https://melete.mneme-ai.space` — `/next` `/discover` `/trust-certificate` `/stability` `/honest-search` `/tolerance` `/improvement` `/prereg` `/breakdown` `/selection` `/support` `/fdr` `/anytime` `/swarm` `/conformal` `/subgroup` `/calibration` `/mcp` `/verify`
